(function(root){
 ...
 function fresh(strategyType='trading'){T.get(strategyType);return {strategyType,format:'strategy-agent-1',source:'',draft:'',messages:[],assessment:null,assessmentContext:null,receipt:null,confirmed:false,versions:[],revision:0,savedAt:null};}
 ...
 function context(s){return {strategyType:s.strategyType,source:s.source,messages:s.messages.map(m=>({role:m.role,content:m.content}))};}
 function plan(p){
 ...
 function applyAssessment(s,r,ctx){if((r.assessment?.strategyType||'trading')!==s.strategyType)throw Error('服务返回的策略类型不一致，请更新云函数后重新评估');s.assessment=plan(r.assessment);s.receipt=r.receipt;s.assessmentContext=ctx;s.confirmed=false;
 ...
 function restore(raw){
  ...
  const s=fresh(raw.strategyType||'trading');s.source=raw.source;s.draft=typeof raw.draft==='string'?raw.draft.slice(0,40000):'';s.messages=raw.messages.map(m=>({role:m.role,content:m.content}));
 ...
 const api={types:T.types,setStrategyType,fresh,invalidate,send,context,plan,applyAssessment,result,addVersion,restore,labels};
