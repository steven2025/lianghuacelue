(function(root){
 ...
 const D=typeof module!=='undefined'&&module.exports?require('./agent_dialogue'):root.AgentDialogue;
 ...
 function context(s){return {...(s.dialogue?{dialogue:D.validate(s.dialogue)}:{}),strategyType:s.strategyType,confirmedNotes:s.confirmedNotes||'',source:s.source,messages:s.messages.map(m=>({role:m.role,content:m.content}))};}
 function plan(p){
 ...
 function restore(raw){
  ...
  s.assessment=raw.assessment?plan(raw.assessment):null;if(raw.dialogue)s.dialogue=D.validate(raw.dialogue);
 ...
 function setStrategyType(s,type){T.get(type);if(s.strategyType===type)return false;s.strategyType=type;if(s.dialogue)s.dialogue=D.fresh();invalidate(s);return true;}
 ...
 function enableDialogue(s){if(!s.dialogue){s.dialogue=D.fresh();invalidate(s);}}
 function deferRule(s){const id=s.assessment?.question?.ruleId,r=s.dialogue?.rules.find(x=>x.id===id);if(!r)throw Error('没有可暂后处理的问题');send(s,'暂后处理：'+r.text);r.status='deferred';}
 function resumeRule(s,id){const r=s.dialogue?.rules.find(x=>x.id===id);if(!r)throw Error('规则不存在');send(s,'现在处理：'+r.text);r.status='missing';s.dialogue.attempts[id]=0;}
 function acceptRule(s,id){const r=s.dialogue?.rules.find(x=>x.id===id);if(r?.status!=='proposed')throw Error('该规则不是待接受建议');send(s,'我确认采用：'+r.text);r.status='confirmed';r.evidence=[{source:'message:'+(s.messages.length-1),quote:s.messages.at(-1).content}];}
 const api={enableDialogue,deferRule,resumeRule,acceptRule,setConfirmedNotes,types:T.types,setStrategyType,fresh,invalidate,send,context,plan,applyAssessment,result,addVersion,restore,labels};
