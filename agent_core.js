(function(root){
 ...
 function fresh(){return {format:'strategy-agent-1',source:'',draft:'',messages:[],assessment:null,assessmentContext:null,receipt:null,confirmed:false,versions:[],revision:0,savedAt:null};}
 ...
 function context(s){return {source:s.source,messages:s.messages.map(m=>({role:m.role,content:m.content}))};}
 function plan(p){
  if(!p||typeof p.name!=='string'||typeof p.summary!=='string'||typeof p.reply!=='string'||!Array.isArray(p.board)||p.board.length!==6||!Array.isArray(p.blockers)||p.blockers.some(x=>typeof x!=='string')||typeof p.ready!=='boolean')throw Error('确认看板格式不正确');
 ...
 function applyAssessment(s,r,ctx){s.assessment=plan(r.assessment);s.receipt=r.receipt;s.assessmentContext=ctx;s.confirmed=false;
 ...
 function restore(raw){
  ...
  const s=fresh();s.source=raw.source;s.draft=typeof raw.draft==='string'?raw.draft.slice(0,40000):'';s.messages=raw.messages.map(m=>({role:m.role,content:m.content}));
 ...
 const api={fresh,invalidate,send,context,plan,applyAssessment,result,addVersion,restore,labels};
