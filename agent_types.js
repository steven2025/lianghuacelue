(function(root){
 ...
 const types={
  ...
  pool:{title:'选股＋剔除退出',description:'更新股票池，仅卖出被剔除持仓',labels:{universe:'股票范围',filters:'排除与筛选条件',ranking:'评分、名额与不足处理',timing:'股票池更新时间',output:'名单与更新记录',exit:'剔除持仓退出',backtest:'历史验证与持仓来源'},example:'每月最后一个交易日收盘后重新评分更新股票池，只卖出实际持仓中被新池剔除的股票，在下一个交易日9:30提交卖出，不自动买入新入选股票。同类依排名递补，合格股票全部用尽仍不足时暂停整个更新、保留原池并提示人工处理，不执行本次更新引起的卖出。'},
