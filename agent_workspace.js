...
(function(){
 ...
 function renderAgent(){
  ...
  root.innerHTML=`<div class="ag-layout">
   ...
   ${a?.ready?`<section class="ag-ready"><b>${ready?'策略方案已整理好':'已恢复方案，需要重新评估'}</b><p>${ready?'快速生成模式：关键条件已确认，其他技术项采用透明默认。':'问答和代码均已保留；重新评估后再确认生成。'}</p>${btn(ready?'agent-confirm':'agent-reassess',ready?(busy&&lastAction==='agent_generate'?'代码生成中…':project.confirmed?'重新生成代码':'确认方案并生成代码'):'重新评估方案',true,busy)}</section>`:''}
