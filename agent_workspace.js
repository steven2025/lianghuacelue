...
(function(){
 ...
 document.querySelector('.badge').textContent='1.0 双模式';
 ...
 function renderAgent(){
  ...
  root.innerHTML=`<div class="ag-layout">
   <aside class="ag-rail"><div class="ag-rail-title"><span class="ag-spark">✦</span><div><b>策略代理</b><small>股票 · 历史回测</small></div></div><p class="ag-rail-caption">一起把交易想法说明白</p><ol class="ag-steps">${['导入策略','逐条补充','确认方案','代码与解释'].map((x,i)=>`<li class="${i===stage?'current':i<stage?'done':''}"><span>${i<stage?'✓':i+1}</span><div>${x}<small>${['粘贴文本或上传文档','每次只回答一个问题','核对六项策略内容','下载后在掘金回测'][i]}</small></div></li>`).join('')}</ol><div class="ag-rail-actions">${btn('agent-new','＋ 新建代理策略')}${btn('agent-load','导入已保存项目')}<input hidden id="agent-project-file" type="file" accept=".json"></div><div class="ag-history"><h3>代码版本 <span>${project.versions.length}</span></h3>${project.versions.length?project.versions.map((x,i)=>`<button data-version="${escape(x.id)}" class="${v?.id===x.id?'selected':''}"><b>版本 ${i+1}</b><small>${x.kind==='repair'?'日志修复':'首次／重新生成'} · ${new Date(x.createdAt).toLocaleDateString('zh-CN')}</small></button>`).reverse().join(''):'<p>生成后自动保留最近6个版本</p>'}</div><div class="ag-rail-foot">${btn('agent-settings','连接与模板')}<small id="agent-saved">${escape(saved||'代理项目独立保存')}</small></div></aside>
   <section class="ag-conversation"><header class="ag-heading"><div><span class="ag-eyebrow">DEEPSEEK · 策略编写助手</span><h1>${escape(a?.name||'用文字描述你的股票策略')}</h1><p>${project.source?'回答会自动记录，也可以随时提出修改。':'无需填写规则表。先说说你想买什么、什么时候买卖。'}</p></div><span class="ag-status"><i></i>${busy?'处理中':'代理模式'}</span></header>
   ...
   <aside class="ag-board"><header><div><span class="ag-eyebrow">策略工作区</span><h2>确认看板 <em>${count}/6</em></h2></div><span class="ag-board-caption">随对话更新</span></header><div class="ag-tabs"><button data-tab="plan" aria-selected="${tab==='plan'}">策略方案</button><button data-tab="code" aria-selected="${tab==='code'}">代码与解释${v?' ●':''}</button></div>${tab==='plan'?`<div class="ag-board-content">${a?`<p class="ag-summary">${escape(a.summary)}</p>`:'<p class="ag-summary">你的策略会逐步整理在这里，随时可以点开修改。</p>'}${Object.entries(C.labels).map(([key,title],i)=>{const r=a?.board.find(x=>x.key===key);return `<article class="ag-card ${r?.status||'empty'}"><div><span class="ag-card-number">0${i+1}</span><h3>${title}</h3><span class="ag-tag">${{confirmed:'已明确',default:'采用默认值',missing:'需要补充'}[r?.status]||'等待整理'}</span></div><p>${escape(r?.content||['股票范围与筛选条件','入场信号与交易时间','退出条件与止损止盈','总仓位、单股与行业限制','成交、停牌及数据不足处理','时间区间、资金与费用'][i])}</p>${r?`<button data-edit="${key}" ${busy?'disabled':''}>用文字修改 ↗</button>`:''}</article>`;}).join('')}<div class="ag-boundary"><b>历史回测</b><span>信息齐备 → 代码检查 → 掘金运行</span><p>三个状态分别验证，不把生成代码当成回测成功。</p></div></div>`:renderCode(v)}
   </aside></div>`;
 ...
 function bindAgent(){
  ...
  root.querySelectorAll('[data-edit]').forEach(el=>el.onclick=()=>{composer='关于“'+C.labels[el.dataset.edit]+'”，我想修改为：';persist();renderAgent();$('agent-message').focus();});
  root.querySelectorAll('[data-example]').forEach(el=>el.onclick=()=>{if(el.dataset.example==='own'){$('agent-file').click();return;}composer='教学回测策略：只交易SHSE.600000和SZSE.000001。每天收盘价高于20日均线时，空仓买入；低于20日均线时卖出可卖持仓。有仓不加仓。总仓位40%，单股最多20%。';persist();renderAgent();$('agent-message').focus();});
 }
