...
function invalidate(){state.revision++;state.confirmed=false;state.generated=null;state.review=null;}
...
function exportCode(){if(!state.generated||!state.confirmed||errors().length){go('audit');notify('请先解决检查项，确认规则后生成代码。');return;}download('main.py',state.generated.code);}
function render(){
 ...
 if(state.view==='language'){
  ...
  html+=section('开始填写',`<div class="actions">${button('template','下载简易模板')}${button('example','载入教学示例','green')}<a class="link" href="策略文本填写模板.md" download>完整组合模板</a>${button('restore','恢复上次项目')}</div><p>本地解析支持简易模板；完整组合模板与自由文本使用后端解析服务。生成器支持均线、涨跌幅、布林线条件的全部/任一组合。其他机制列为待解决项。</p><div id="drop" class="dropzone"><strong>选择或拖入 TXT、MD、项目JSON</strong><input id="file" type="file" accept=".txt,.md,.json"><small>DOCX、Excel请先粘贴正文。最大1MB。</small></div>`);
  html+=section('策略原文',`<textarea id="text" class="input" rows="15" placeholder="填写或导入策略文本">${esc(state.text)}</textarea><div class="actions">${button('parse','本地解析模板')}</div><p>前端不保存或直连大模型密钥。需要AI解析时由后端接口完成。</p>`);
